...
import { generateADPExport } from '@/lib/integrations/adp/export'
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const csv = await generateADPExport(orgId, startDate, endDate)
    const filename = `adp_timesheet_${startDate}_${endDate}.csv`

    await writeSyncEvent({
      orgId, integrationType: 'adp', triggeredBy: user.id,
      status: 'success', recordsSynced: csv.split('\n').length - 1,
      details: { start_date: startDate, end_date: endDate },
      auditAction: 'ADP_EXPORT',
    })
