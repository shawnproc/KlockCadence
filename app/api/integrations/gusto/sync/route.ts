...
import { syncLeaveToGusto } from '@/lib/integrations/gusto/sync'
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result = await syncLeaveToGusto(orgId, body.start_date, body.end_date)
    ...
    await writeSyncEvent({
      orgId, integrationType: 'gusto', triggeredBy: user.id,
      status: 'error', recordsSynced: 0, errorMessage: message, auditAction: 'GUSTO_SYNC',
    })
