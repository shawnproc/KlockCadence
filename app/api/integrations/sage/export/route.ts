...
import { generateSageExport } from '@/lib/integrations/sage/export'
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const csv = await generateSageExport(orgId, startDate, endDate)
    const filename = `sage_intacct_${startDate}_${endDate}.csv`

    await writeSyncEvent({
      orgId, integrationType: 'sage_intacct', triggeredBy: user.id,
      status: 'success', recordsSynced: csv.split('\n').length - 1,
      details: { start_date: startDate, end_date: endDate },
      auditAction: 'SAGE_EXPORT',
    })
